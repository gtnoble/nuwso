...
class Planet
  ...
  def surface_integral(sampler, tolerance)
  
    integrand = Proc.new do |latitude, longitude|
      spherical_surface_factor =    @radius ** 2 *
                                    Math.cos(latitude * 
                                             DEGREES_TO_RADIANS) *
                                    DEGREES_TO_RADIANS ** 2 

      yield(latitude, longitude) * spherical_surface_factor 
    end
  end
end
